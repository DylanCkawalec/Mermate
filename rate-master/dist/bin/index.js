...
const RateMaster_1 = require("../src/RateMaster");
...
// ─── Bootstrap ────────────────────────────────────────────────────────────────
const rm = new RateMaster_1.RateMaster({
    endpoints: {
        "local-ai": {
...
function fakeInference(endpoint, latencyMs, priority) {
    rm.execute(endpoint, () => new Promise((res) => setTimeout(() => res("ok"), latencyMs)), { priority }).catch(() => undefined);
}
function spawnDemoLoad() {
    // Local AI: continuous low-volume critical + background mix
    setInterval(() => fakeInference("local-ai", 300 + Math.random() * 500, types_1.JobPriority.CRITICAL), 800);
    setInterval(() => fakeInference("local-ai", 200 + Math.random() * 300, types_1.JobPriority.BACKGROUND), 400);
    // External API: sporadic high-priority bursts
    setInterval(() => fakeInference("external-api", 100 + Math.random() * 200, types_1.JobPriority.HIGH), 1_200);
    setInterval(() => fakeInference("external-api", 50 + Math.random() * 100, types_1.JobPriority.NORMAL), 2_500);
}
// ─── ANSI dashboard ───────────────────────────────────────────────────────────
...
function renderDashboard() {
    ...
    lines.push(`${BOLD}${CYAN}║         rate-master  ·  live dashboard               ║${RESET}`);
    lines.push(`${BOLD}${CYAN}╚══════════════════════════════════════════════════════╝${RESET}`);
    lines.push(`  uptime ${CYAN}${formatMs(m.uptimeMs)}${RESET}   ` +
    ...
    for (const [name, ep] of Object.entries(m.endpoints)) {
        ...
        lines.push(`    window  ${bar(ep.windowUsage, ep.currentLimit)} ` +
            `${utilColor}${ep.windowUsage}/${ep.currentLimit}${RESET} req`);
        lines.push(`    queue   ${YELLOW}${ep.queueDepth.toString().padStart(4)}${RESET}  ` +
            `active ${GREEN}${ep.activeRequests}${RESET}  ` +
            `executed ${ep.totalExecuted}  ` +
            `errors ${ep.totalErrors > 0 ? RED : ""}${ep.totalErrors}${RESET}`);
        lines.push(`    latency p50=${CYAN}${formatMs(ep.executionLatency.p50)}${RESET} ` +
        ...
        lines.push(`    errRate ${ep.errorRate > 0.05 ? RED : GREEN}${(ep.errorRate * 100).toFixed(1)}%${RESET}  ` +
    ...
    if (recentDecisions.length > 0) {
        ...
        for (const d of recentDecisions) {
            const arrow = d.reason === "scale-up" ? GREEN + "▲" : d.reason === "scale-down" ? RED + "▼" : DIM + "─";
            lines.push(`  ${arrow}${RESET} ${d.endpoint.padEnd(14)} ${d.previousLimit} → ${BOLD}${d.newLimit}${RESET}  ` +
